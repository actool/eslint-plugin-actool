    ...
    scope.block.body.forEach((statement) => {
        ...
        if (options.line) {
            ...
            innerComments.forEach((innerComment) => {
                ...
                if (diffByCommits >= 1) {
                    context.report({
                        message: `At line ${lineCommit.line} occured inrelevant comment by ${diffByCommits} commits`,
        ...
        if (options.block) {
            ...
            if (diffByCommits >= 1) {
                context.report({
                    message: `At block [${comment.loc.start.line}-${comment.loc.end.line}] occured inrelevant comment block by ${diffByCommits} commits`,
