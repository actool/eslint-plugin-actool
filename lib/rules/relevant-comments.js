...
function create(context) {
    ...
    try {
        ...
        scope.block.body.forEach((statement, index) => {
            console.log(`RELEVANT-COMMENT: [${index}] ${statement.loc.start.line}-${statement.loc.end.line}`);
            ...
            if (options.line) {
                ...
                innerComments.forEach((innerComment) => {
                    ...
                    const lineCommit = revision.getLineCommit({ file, line: tokenAfter.loc.start.line });
                    ...
                    if (diffByDate >= 1) {
                        ...
                        context.report({
                            // FIXME: later will be compute by days
                            message: `For line ${lineCommit.line} occurred irrelevant comment by ${(diffByDate).toFixed(0)} min`,
                            loc: tokenAfter.loc,
                        })
                    }
                })
            ...
            if (options.block) {
                ...
                if (diffByDate >= 1) {
                    ...
                    context.report({
                        // FIXME: later will be compute by days
                        message: `For block [${comment.loc.start.line}-${comment.loc.end.line}] occurred irrelevant comment by ${(diffByDate).toFixed(0)} min`,
                        loc: comment.loc,
                    })
                }
            }
        ...
        console.error(`Failed to analyze ${file}.\n TIP: Please, send log and screen of trouble file in issue of eslint-plugin-actool =)`, JSON.stringify(e));
    }
